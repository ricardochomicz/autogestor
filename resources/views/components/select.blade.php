...
<div>
    ...
    @endif
    <select id="{{ $id }}" class="selectpicker" data-live-search="true" name="{{ $name }}"
        {{ $attributes->class(['is-invalid' => $errors->has(str_replace('[]', '', $name))]) }}
        ...
        @foreach ($options as $op)
            ...
            @if (isset($attributes['multiple']))
                ...
                @endphp
                <option data-content="<span class='badge badge-light' style='font-size:14px'>{{ $op['name'] }}</span>"
                    value="{{ $idOP }}" @if (in_array($idOP, $selectedValues))
                    selected
            @endif>{{ $op['name'] }}</option>
        @else
            <option value="{{ $idOP }}" @selected($value == $idOP)>{{ $op->name }} @if (isset($op->text))
                    {{ $op->text }}
                @endif
            </option>
        @endif
